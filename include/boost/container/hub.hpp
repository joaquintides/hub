...
public:
   ...
   //!   The memory of a slab-carved block is only returned to the allocator when
   //!   all the blocks of its slab have been deallocated, so the reduction in
   //!   capacity() is not necessarily matched by an equivalent release of
   //!   allocated memory.
   void shrink_to_fit()
   {
      compact();
   ...
   //!
   //! <b>Note</b>: All references, pointers and iterators (including the
   //!   past-the-end iterator) remain valid. The memory of a slab-carved block
   //!   is only returned to the allocator when all the blocks of its slab have
   //!   been deallocated, so the reduction in capacity() is not necessarily
   //!   matched by an equivalent release of allocated memory.
   ...
   //!   blocks of *this.
   //!
   //! <b>Note</b>: Transferred blocks carved from a slab of x keep their
   //!   association with that slab: the slab's memory is returned to the
   //!   allocator, by whichever container deallocates its last block, only when
   //!   all its blocks have been deallocated, regardless of the containers those
   //!   blocks were transferred to. x retains its partially carved slab, if any,
   //!   and will preferentially use its spare capacity for subsequently created
   //!   blocks.
   void splice(hub& x)
   {
      BOOST_ASSERT(this != &x);
...
private:
   ...
   using slab_storage_allocator =
      hub_detail::rebind_alloc_t<allocator_type, slab_storage>;
   using slab_storage_alloc_traits = allocator_traits<slab_storage_allocator>;
   using slab_storage_pointer = typename slab_storage_alloc_traits::pointer;
   ...
   static block_pointer slab_block_at(slab_base* s, std::size_t i) noexcept
   {
      ...
      return hub_detail::pointer_traits<block_pointer>::pointer_to(*pb);
   }
   static pointer slab_data_at(slab_base* s, std::size_t i) noexcept
   {
      auto pd = reinterpret_cast<T*>(
         ...
         i * (std::size_t)N;
      return hub_detail::pointer_traits<pointer>::pointer_to(*pd);
   }

   slab_base_pointer allocate_slab()
   {
      slab_storage_allocator sal(al());
      /* may throw, state unchanged */
      auto ps = slab_storage_alloc_traits::allocate(sal, 1);
      auto s = reinterpret_cast<slab_base*>(boost::movelib::to_raw_pointer(ps));
      s->live = 0;
      s->used = 0;
      s->open = true;
      return hub_detail::pointer_traits<slab_base_pointer>::pointer_to(*s);
   }

   void deallocate_slab(slab_base* s) noexcept
   {
      slab_storage_allocator sal(al());
      slab_storage_alloc_traits::deallocate(
         sal,
         hub_detail::pointer_traits<slab_storage_pointer>::pointer_to(
   ...
   {
      if(cur_slab) {
         auto s = boost::movelib::to_raw_pointer(cur_slab);
         s->open = false;
   ...
   block_pointer create_new_available_block()
   {
      ...
      if(
         cur_slab &&
         boost::movelib::to_raw_pointer(cur_slab)->used < slab_blocks) {
         ...
         pb = block_alloc_traits::allocate(al(), 1);
         pb->mask = 0;
         BOOST_CONTAINER_TRY {
   ...
   {
      if(pb->owner) {
         auto s = boost::movelib::to_raw_pointer(pb->owner);
